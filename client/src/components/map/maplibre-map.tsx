  ...
  useEffect(() => {
    ...
    try {
      ...
      map.current.once('load', () => {
        ...
        // DISABLED FOR iOS: Satellite sources disabled to prevent WebView memory crash
        console.log('[SATELLITE] Satellite sources disabled for iOS stability');

        // DISABLED FOR iOS: Satellite layers disabled to prevent WebView memory crash
        console.log('[SATELLITE] Satellite layers disabled for iOS stability');

        // DISABLED FOR iOS: Labels overlay disabled to prevent WebView memory crash
        console.log('[LABELS] Labels overlay disabled for iOS stability');

        // DISABLED FOR iOS: 3D buildings disabled to prevent WebView memory crash
        console.log('[3D-BUILDINGS] 3D buildings disabled for iOS stability');

        setIsLoaded(true);
        console.log('[MAP-INIT] ✅ MapLibre GL LOADED - isLoaded set to true');
        
        // DISABLED FOR iOS: Satellite prefetch disabled to prevent WebView memory crash
        console.log('[SATELLITE-PREFETCH] Prefetch disabled for iOS stability');
      });
